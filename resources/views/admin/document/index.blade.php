  ...
  <div class="page-content">
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        <div class="col-lg-4">
          <div class="card">
            <div class="card-header"><h4 class="mb-0">Add Document</h4></div>
            <div class="card-body">
              <form method="POST" action="{{ route('admin.document.store') }}" enctype="multipart/form-data" id="documentForm">
                @csrf
              ...
              <div class="mb-3">
                ...
                <input type="file" name="file" class="form-control"
                       accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip" required>
                <small class="text-muted">Allowed: PDF, DOCX, XLSX, PPTX, TXT, CSV, ZIP (Max 20MB)</small>
              </div>
        ...
        <div class="col-lg-8">
          <div class="card">
            <div class="card-header">
              <form method="GET" class="row g-2 align-items-center" action="{{ route('admin.document.index') }}">
                <div class="col-md-3">
                ...
                <div class="col-md-2 d-flex gap-2">
                  ...
                  <a href="{{ route('admin.document.index') }}" class="btn btn-light w-100">Reset</a>
                </div>
              </form>
            </div>
            ...
            <div class="card-body">
              <div class="d-flex justify-content-end mb-2">
                <form method="POST" id="bulkDeleteForm" action="{{ route('admin.document.bulk-delete') }}">
                  @csrf
                  <button type="submit" class="btn btn-danger btn-sm" id="bulkDeleteBtn"><i class="fas fa-trash"></i> Delete All</button>
                </form>
              </div>
              ...
              <div class="table-responsive">
                <table class="table table-bordered table-striped align-middle">
                  ...
                  <tbody>
                    @forelse($list as $row)
                      <tr>
                        <td><input type="checkbox" name="ids[]" form="bulkDeleteForm" value="{{ $row->document_id }}"></td>
                        <td>
                          @if($row->document)
                            <a href="{{ asset('anvixa/'.$row->document) }}" target="_blank" class="btn btn-sm btn-outline-primary">
                              <i class="fas fa-download me-1"></i> View / Download
                            </a>
                            <div><small class="text-muted">{{ basename($row->document) }}</small></div>
                          @else
                            —
                          @endif
                        </td>
                        ...
                        <td class="text-end">
                          ...
                          <form method="POST" action="{{ route('admin.document.destroy', $row->document_id) }}" class="d-inline">
                            @csrf @method('DELETE')
              ...
              <div class="modal-body">
                ...
                <div class="row g-3">
                  ...
                  <div class="col-md-6">
                    ...
                    <input type="file" name="file" class="form-control"  accept=".pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip" >
