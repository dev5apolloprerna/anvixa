                            ...
                            <div class="card-body">
                                <form method="POST" action="{{ route('admin.document.store') }}"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <small class="text-muted">Allowed: PDF, DOCX, XLSX, PPTX, TXT, CSV, ZIP (Max
                                            20MB)</small>
                                    </div>
                        ...
                        <div class="card">
                            <div class="card-header">
                                <form method="GET" class="row g-2 align-items-center"
                                    action="{{ route('admin.document.index') }}">
                                    <div class="col-md-3">
                                    ...
                                    <div class="col-md-3">
                                        <select class="form-control" name="subcategory_id" id="filter_subcategory">
                                            ...
                                            @foreach ($subcategories as $sid => $sname)
                                                <option value="{{ $sid }}" @selected($subcatId == $sid)>
                                                    {{ $sname }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        <input type="text" class="form-control" name="q"
                                            value="{{ $q }}" placeholder="Search title / slug">
                                    </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-striped align-middle">
                                        ...
                                        <tbody>
                                            @forelse($list as $row)
                                                <tr>
                                                    <td><input type="checkbox" name="ids[]" form="bulkDeleteForm"
                                      ...
                                      <span class="badge {{ $row->iStatus ? 'bg-success' : 'bg-secondary' }}">
                                        {{ $row->iStatus ? 'Active' : 'Inactive' }}
                                                    ...
                                                    <td class="text-end">
                                                        ...
                                                        <form method="POST"
                                                            action="{{ route('admin.document.destroy', $row->document_id) }}"
                                                            class="d-inline">
                                                            @csrf @method('DELETE')
                                        ...
                                        <button class="btn btn-sm btn-outline-dark">
                                          {{ $row->iStatus ? 'Deactivate' : 'Activate' }}
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="7" class="text-center text-muted">No documents found
                                                    </td>
                                                </tr>
                                            @endforelse
                                        </tbody>
    ...
    <script>
        (function() {
            ...
            document.getElementById('title')?.addEventListener('input', function() {
                ...
                const t = document.getElementById('slugPreview');
            ...
            document.getElementById('editTitle')?.addEventListener('input', function() {
                const s = this.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
                const t = document.getElementById('editSlugPreview');
                if (t) t.textContent = s;
            });
